/**
 * Formulário de criação / edição de transações (sem chrome próprio).
 *
 * Renderiza apenas o `<form>` — espera-se que o consumidor coloque-o dentro
 * de um modal (`<Modal>`) ou outro contentor com cabeçalho próprio.
 *
 * O modo (criar vs editar) é derivado de `editingId`: quando é uma string,
 * carrega os dados da transação correspondente e mostra acções de "Salvar /
 * Voltar"; quando é `null`, mostra apenas "Incluir".
 *
 * `onClose()` é invocado:
 *  - ao clicar em "Voltar" / fechar
    ...
    >
      ...
      <div className="modal-action col-span-full flex justify-end gap-2 min-[640px]:col-span-2">
        ...
        >
          ...
          <span>
            {submittingTx ? "Salvando…" : editing ? "Salvar" : "Incluir"}
          </span>
        </button>
      </div>
    </form>
