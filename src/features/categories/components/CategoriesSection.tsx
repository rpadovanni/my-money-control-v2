    ...
    <section className="mt-4">
      <div className="card border border-base-300 bg-base-100">
        ...
        <p className="text-sm text-base-content/70">
          ...
          tudo retroativamente. A categoria &quot;Transferência&quot; é
        ...
        {!catReady ? (
          ...
          <ul className="mt-4 space-y-2">
            {categories.map((c) => (
              <li key={c.id} className="rounded-box border border-base-300 bg-base-100 p-3">
                  ...
                  {categoryEdit?.id === c.id ? (
                    ...
                    <>
                      <div className="min-w-0 flex-1">
                        ...
                        {c.system ? <span className="badge badge-outline">Sistema</span> : null}
                      </div>
                    </>
                  )}
                  {categoryEdit?.id !== c.id ? (
                    <div className="flex shrink-0 flex-col items-end gap-2">
                      <span className="text-xs text-base-content/60" title={c.id}>
                        {c.id}
                      </span>
                      {!c.system ? (
